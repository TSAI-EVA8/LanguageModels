...
class AttentionHead(nn.Module):
    ...
    """
    ## num_embed= initial input size say 512
    ## head_size : the projection output say 64
    def __init__(self, head_size, num_embed, block_size, dropout):
        super().__init__()
        self.key = nn.Linear(num_embed, head_size, bias=False)
        self.query = nn.Linear(num_embed, head_size, bias=False)
        self.value = nn.Linear(num_embed, head_size, bias=False)
        # compute the number of sparse connections for each head
        sparsity=0.5
        num_sparse = int(sparsity * head_size)
        
        # create a matrix to indicate the sparse connections
        self.sparse_mask = torch.zeros(head_size, head_size)
        for i in range(head_size):
            # randomly choose a set of indices for sparse connections
            indices = torch.randperm(head_size)[:num_sparse]
            self.sparse_mask[i, indices] = 1
    ...
    def forward(self, x):
        ...
        #print("sparse matric",self.sparse_mask.shape)
        
         # apply the sparse mask to the keys
        k = k.matmul(self.sparse_mask.to(DEVICE))

        #print("After the sparse",k.shape)
...
class AttentionHead_(nn.Module):
    """
    One head of the self-attention layer
    """
